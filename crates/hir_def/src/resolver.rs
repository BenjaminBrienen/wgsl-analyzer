...
fn resolve_import_item(
    database: &dyn DefDatabase,
    anchor_file_id: EditionedFileId,
    import: &ImportStatement,
    flat_import: &crate::item_tree::FlatImport,
) -> Option<(EditionedFileId, ModuleItem)> {
    ...
    let anchor_file = anchor_file_id.file_id(database);
    let target_file = resolve_import_to_file(database, anchor_file, import.kind, module_segments)?;
    let target_editioned = EditionedFileId::from_file(database, target_file);
    let target_tree = database.item_tree(target_editioned);
    ...
    Some((target_editioned, *target_item))
}
