...
use base_db::{Lookup, SourceDatabase};
use syntax::{Edition, ast::HasName as _};

use super::{Expression, ExpressionId, ExpressionStore, TypeSpecifierId, ast};
use crate::{
    HasSource, InFile,
    body::{Binding, BindingId, Body, BodySourceMap},
    db::{DefinitionWithBodyId, FunctionId, ModuleDefinitionId, StructId},
...
fn name_to_string(
    ...
    name.and_then(syntax::ast::Name::ident_token)
        .map_or_else(|| default.to_owned(), |token| token.to_string())
}
...
pub fn print_function(
    ...
    write!(printer, "{}", name.as_str());
    write!(printer, "(");
    for (index, (parameter, parameter_data)) in parameters.iter().enumerate() {
        if index != 0 {
            write!(printer, ", ");
        }
...
impl Printer<'_> {
    fn indented(
        &mut self,
        print_indented: impl FnOnce(&mut Self),
    ) {
    ...
    fn newline(&mut self) {
        if matches!(self.line_format, LineFormat::Oneline) {
            ...
            match self.buffer.chars().rev().find_position(|ch| *ch != ' ') {
                ...
                Some((index, _)) => {
                    if index != 0 {
                        self.buffer.drain(self.buffer.len() - index..);
                    }
                    write!(self, "\n");
                },
            }
    ...
    fn print_expression(
        ...
        match expression {
            ...
            Expression::IdentExpression(identifier_expression) => {
                self.print_path(&identifier_expression.path);
            },
            Expression::Call {
                ident_expression,
    ...
    #[expect(
        ...
        reason = "sometimes a function has a lot of meat on its bones and we just have to accept that"
    )]
    fn print_statement(
        &mut self,
        body: &Body,
        ...
        match statement {
            ...
            Statement::Variable {
                ...
                write!(self, "var");
                self.print_template(template_parameters);
                self.whitespace();
                self.print_binding(body, *binding_id);
                if let Some(type_ref) = type_ref {
                    write!(self, ": ");
    ...
    pub(crate) fn print_template(
        ...
        write!(self, "<");
        let mut first = true;
        for argument in template_parameters {
            if !first {
                write!(self, ", ");
            }
            first = false;
