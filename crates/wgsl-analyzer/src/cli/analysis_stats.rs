...
use base_db::{EditionedFileId, Package, SourceDatabase, all_packages};
use edition::Edition;
use hir_def::{body::BodySourceMap, database::DefinitionWithBodyId, expression::ExpressionId, expression_store::SyntheticSyntax, item_tree::Name};
use ide::{AnalysisHost, LineCol, RootDatabase};
...
impl flags::AnalysisStats {
    pub fn run(
        ...
        let manifest = ProjectManifest::discover(&path, false).unwrap();
        ...
        let mut packages = hir(db);
        if self.randomize {
            shuffle(&mut rng, &mut packages);
        }

        let mut item_tree_sw = self.stop_watch();
        let source_roots = packages
            .iter()
            .cloned()
            .map(|package| {
                db.file_source_root(package.root_file(db))
                    .source_root_id(db)
            })
            .unique();
        ...
        for source_root_id in source_roots {
            ...
            for file_id in source_root.iter() {
                    ...
                    && let Some((_, Some("rs"))) = p.name_and_extension()
                {
                    ...
                    if !source_root.is_library || self.with_dependencies {
        ...
        while let Some(module) = visit_queue.pop() {
            if visited_modules.insert(module) {
                ...
                for decl in module.declarations(db) {
                    num_decls += 1;
                    match decl {
                        ModuleDefinition::Function(f) => bodies.push(DefinitionWithBody::from(f)),
                        ModuleDefinition::Adt(a) => {
                            if let Adt::Enum(e) = a {
                                for v in e.variants(db) {
                                    bodies.push(DefinitionWithBody::from(v));
                                }
                            }
                            adts.push(a)
                        },
                        ModuleDefinition::Const(c) => {
                            bodies.push(DefinitionWithBody::from(c));
                        },
                        ModuleDefinition::Static(s) => bodies.push(DefinitionWithBody::from(s)),
                        ModuleDefinition::Trait(_) => num_traits += 1,
                        ModuleDefinition::Macro(m) => match m.kind(db) {
                ...
                for impl_def in module.impl_defs(db) {
                    for item in impl_def.items(db) {
                        num_decls += 1;
                        match item {
                            AssocItem::Function(f) => bodies.push(DefinitionWithBody::from(f)),
                            AssocItem::Const(c) => {
                                bodies.push(DefinitionWithBody::from(c));
                            },
                            _ => (),
        ...
        eprintln!(
            ...
            bodies
                .iter()
                .filter(|it| matches!(it, DefinitionWithBody::Const(_) | DefinitionWithBody::Static(_)))
                .count(),
        );
    ...
    fn run_const_eval(
        &self,
        db: &RootDatabase,
        bodies: &[DefinitionWithBody],
        verbosity: Verbosity,
    ) {
        let len = bodies
            .iter()
            .filter(|body| matches!(body, DefinitionWithBody::Const(_) | DefinitionWithBody::Static(_)))
            .count();
        ...
        for &b in bodies {
            ...
            let res = match b {
                DefinitionWithBody::Const(c) => c.eval(db),
                DefinitionWithBody::Static(s) => s.eval(db),
                _ => continue,
            };
            bar.inc(1);
            all += 1;
    ...
    fn run_mir_lowering(
        ...
        for &body_id in bodies {
            ...
            bar.inc(1);
            if matches!(body_id, DefinitionWithBody::Variant(_)) {
                continue;
            }
            ...
            let msg = move || {
                if verbosity.is_verbose() {
                    let source = match body_id {
                        DefinitionWithBody::Function(it) => it.source(db).map(|it| it.syntax().cloned()),
                        DefinitionWithBody::Static(it) => it.source(db).map(|it| it.syntax().cloned()),
                        DefinitionWithBody::Const(it) => it.source(db).map(|it| it.syntax().cloned()),
                        DefinitionWithBody::Variant(it) => it.source(db).map(|it| it.syntax().cloned()),
                    };
...
impl fmt::Display for PrettyItemStats {
    fn fmt(
        ...
        write!(formatter, "mods: {}", self.mods)
    }
}
