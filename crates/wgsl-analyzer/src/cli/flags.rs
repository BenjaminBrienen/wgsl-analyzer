...
xflags::xflags! {
    ...
    cmd wgsl-analyzer {
        ...
        cmd analysis-stats {
            /// Directory with wesl.toml or wesl-project.json.
            required path: PathBuf

            optional --output format: OutputFormat

            /// Randomize order in which packages, modules, and items are processed.
            optional --randomize
...
pub struct WgslAnalyzer {
    ...
    pub wait_dbg: bool,
    pub subcommand: WgslAnalyzerCmd,
}

#[derive(Debug)]
pub enum WgslAnalyzerCmd {
    LspServer(LspServer),
    Parse(Parse),
    Symbols(Symbols),
    Highlight(Highlight),
