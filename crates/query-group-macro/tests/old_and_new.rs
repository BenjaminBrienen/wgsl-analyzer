...
fn length_query(
    db: &dyn PartialMigrationDatabase,
    input: Input,
) -> usize {
    input.str(db).len()
}

fn invoke_length_query_actual(
    db: &dyn PartialMigrationDatabase,
    input: Input,
) -> usize {
    input.str(db).len()
...
#[salsa::tracked]
fn invoke_length_tracked_actual(
    db: &dyn PartialMigrationDatabase,
    input: Input,
) -> usize {
    input.str(db).len()
