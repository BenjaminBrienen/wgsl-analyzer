...
fn collect_transitive_imports(
    ...
    while let Some(file_id) = queue.pop() {
        ...
        let editioned = EditionedFileId::from_file(database, file_id);
        // Only follow imports in WESL files
        if !editioned.edition(database).at_least_wesl_0_0_1() {
            continue;
        }

        let item_tree = database.item_tree(editioned);
...
fn build_combined_source(
    ...
    for &dep_file in &imported_files {
        let source = database.file_text(dep_file).text(database);
    ...
    let main_source = database.file_text(main_file).text(database);
...
fn naga_diagnostics<N: Naga>(
    ...
    let is_wesl = file_id.edition(database).at_least_wesl_0_0_1();
    ...
    let (source, main_file_offset) = if is_wesl {
        let (combined, _source_map, main_range) =
            build_combined_source(database, file_id.file_id(database));
        (combined, main_range.start)
    } else {
        let text = database
            .file_text(file_id.file_id(database))
            .text(database)
            .to_string();
        (text, 0)
    };
