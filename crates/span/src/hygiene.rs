...
const _: () = {
    ...
    impl zalsa_::AsId for SyntaxContext {
        fn as_id(&self) -> salsa::Id {
            self.as_salsa_id()
                .expect("`SyntaxContext::as_id()` called on a root `SyntaxContext`")
        }
    }
    ...
    impl<'db> SyntaxContext {
        ...
        where
            Db: ?Sized + zalsa_::Database,
        {
            let Some(id) = self.as_salsa_id() else {
        ...
        pub fn opaque_and_semitransparent<Db>(
            self,
            db: &'db Db,
        ) -> SyntaxContext
        where
...
impl SyntaxContext {
    ...
    pub fn is_root(self) -> bool {
        (SyntaxContext::MAX_ID - Edition::LATEST as u32) <= self.into_u32()
            && self.into_u32() <= (SyntaxContext::MAX_ID - Edition::Wgsl as u32)
    }

    #[inline]
    pub fn remove_root_edition(&mut self) {
        if self.is_root() {
            *self = Self::root(Edition::Wgsl);
        }
    }
    ...
    fn fmt(
        ...
        if self.is_root() {
            write!(
                f,
                "ROOT{}",
                Edition::from_u32(SyntaxContext::MAX_ID - self.into_u32())
            )
        } else {
