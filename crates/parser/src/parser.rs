...
impl<'source> ParserCallbacks<'source> for Parser<'source> {
    ...
    fn create_node_let_declaration(
        ...
        let has_eq = self
            .cst
            .children(node_ref)
            .any(|child| self.cst.match_token(child, Token::Equal).is_some());
        if has_name && !has_eq {
            diags.push(self.create_diagnostic(
...
impl Parser<'_> {
    ...
    fn classify_op(token: Token) -> OpClass {
        match token {
            ...
            | Token::EqualEqual
            | Token::XorEqual
            | Token::NotEqual => OpClass::Comparison,
            Token::ShiftLeft | Token::ShiftRight => OpClass::Shift,
            Token::And | Token::Or | Token::Xor => OpClass::Bitwise,
            Token::AndAnd | Token::OrOr => OpClass::Logical,
            _ => OpClass::Other,
        }
    }
}
